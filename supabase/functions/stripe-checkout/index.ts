...
Deno.serve(async (req) => {
  try {
    ...
    // Only check for existing customer if user is authenticated
    if (user) {
    ...
    if (!customer || !customer.customer_id) {
      ...
      // Only create customer mapping if user is authenticated
      if (user) {
      ...
      console.log(`Successfully set up new customer ${customerId}${user ? ' with subscription record' : ' for guest checkout'}`);
    } else {
